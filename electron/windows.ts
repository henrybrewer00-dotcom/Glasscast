...
const WINDOW_ICON_FILENAME =
	process.platform === "darwin" ? "glasscastmac-512.png" : "glasscast-512.png";
...
function getEditorWindowQuery(): Record<string, string> {
	...
	if (process.env.GLASSCAST_DEV_OPEN_RECORDING_INPUT) {
		query.devOpenInput = process.env.GLASSCAST_DEV_OPEN_RECORDING_INPUT;
	}
	if (process.env.GLASSCAST_DEV_OPEN_RECORDING_WEBCAM) {
		query.devOpenWebcam = process.env.GLASSCAST_DEV_OPEN_RECORDING_WEBCAM;
	}

	if (process.env.GLASSCAST_SMOKE_EXPORT === "1") {
		query.smokeExport = "1";
		if (process.env.GLASSCAST_SMOKE_EXPORT_INPUT) {
			query.smokeInput = process.env.GLASSCAST_SMOKE_EXPORT_INPUT;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_OUTPUT) {
			query.smokeOutput = process.env.GLASSCAST_SMOKE_EXPORT_OUTPUT;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_USE_NATIVE === "1") {
			query.smokeUseNativeExport = "1";
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_ENCODING_MODE) {
			query.smokeEncodingMode = process.env.GLASSCAST_SMOKE_EXPORT_ENCODING_MODE;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_SHADOW_INTENSITY) {
			query.smokeShadowIntensity = process.env.GLASSCAST_SMOKE_EXPORT_SHADOW_INTENSITY;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_INPUT) {
			query.smokeWebcamInput = process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_INPUT;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_SHADOW) {
			query.smokeWebcamShadow = process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_SHADOW;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_SIZE) {
			query.smokeWebcamSize = process.env.GLASSCAST_SMOKE_EXPORT_WEBCAM_SIZE;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_PIPELINE) {
			query.smokePipelineModel = process.env.GLASSCAST_SMOKE_EXPORT_PIPELINE;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_BACKEND) {
			query.smokeBackendPreference = process.env.GLASSCAST_SMOKE_EXPORT_BACKEND;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_RENDER_BACKEND) {
			query.smokeRenderBackend = process.env.GLASSCAST_SMOKE_EXPORT_RENDER_BACKEND;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
			query.smokeMaxEncodeQueue = process.env.GLASSCAST_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
			query.smokeMaxDecodeQueue = process.env.GLASSCAST_SMOKE_EXPORT_MAX_DECODE_QUEUE;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
			query.smokeMaxPendingFrames = process.env.GLASSCAST_SMOKE_EXPORT_MAX_PENDING_FRAMES;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_PROJECT) {
			query.smokeProject = process.env.GLASSCAST_SMOKE_EXPORT_PROJECT;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_QUALITY) {
			query.smokeQuality = process.env.GLASSCAST_SMOKE_EXPORT_QUALITY;
		}
		if (process.env.GLASSCAST_SMOKE_EXPORT_FPS) {
			query.smokeFps = process.env.GLASSCAST_SMOKE_EXPORT_FPS;
		}
	}
...
export function createHudOverlayWindow(): BrowserWindow {
	...
	// Keep the recorder orb visible no matter which macOS Space / full-screen app
	// the user is on. Without this, the always-on-top orb is stranded on the Space
	// it was created on and the app appears to open to "nothing".
