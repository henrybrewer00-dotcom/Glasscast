...
type NativeVideoExportHeartbeat = {
	sessionId: string;
	sequence: number;
	atMs: number;
...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	onNativeVideoExportHeartbeat: (
		callback: (heartbeat: NativeVideoExportHeartbeat) => void,
	) => {
		const listener = (
			_event: Electron.IpcRendererEvent,
			payload: NativeVideoExportHeartbeat,
		) => callback(payload);
		ipcRenderer.on("native-video-export-heartbeat", listener);
		return () => ipcRenderer.removeListener("native-video-export-heartbeat", listener);
	},
	...
	getWhisperModelStatus: (modelId?: string) => {
		return ipcRenderer.invoke("get-whisper-model-status", modelId);
	},
	...
	onCaptionModelDownloadProgress: (
		callback: (state: {
			modelId: string;
		...
		ipcRenderer.on("caption-model-download-progress", listener);
		return () => ipcRenderer.removeListener("caption-model-download-progress", listener);
	},
	getCaptionKeyStatuses: () => {
		return ipcRenderer.invoke("get-caption-key-statuses");
	},
	getCaptionKeyStatus: (provider: string) => {
		return ipcRenderer.invoke("get-caption-key-status", provider);
	},
	saveCaptionKey: (options: { provider: string; key: string }) => {
		return ipcRenderer.invoke("save-caption-key", options);
	},
	deleteCaptionKey: (provider: string) => {
		return ipcRenderer.invoke("delete-caption-key", provider);
	},
	getAgentKeyStatuses: () => {
		return ipcRenderer.invoke("get-agent-key-statuses");
	},
	saveAgentKey: (options: { provider: string; key: string }) => {
		return ipcRenderer.invoke("save-agent-key", options);
	},
	deleteAgentKey: (provider: string) => {
		return ipcRenderer.invoke("delete-agent-key", provider);
	},
	invokeAgentChat: (options: {
		provider: string;
		url: string;
		headers: Record<string, string>;
		body: unknown;
	}) => {
		return ipcRenderer.invoke("invoke-agent-chat", options);
	},
	...
	// ── Cloud sync & account ────────────────────────────────────────────
	cloudGetStatus: () => ipcRenderer.invoke("cloud:status"),
	cloudSignIn: (email: string, password: string) =>
		ipcRenderer.invoke("cloud:sign-in", email, password),
	cloudSignUp: (email: string, password: string) =>
		ipcRenderer.invoke("cloud:sign-up", email, password),
	cloudSignOut: () => ipcRenderer.invoke("cloud:sign-out"),
	cloudSyncNow: () => ipcRenderer.invoke("cloud:sync-now"),
	cloudNotifySettingsChanged: () => ipcRenderer.invoke("cloud:settings-changed"),
	cloudPushPresets: (
		presets: { kind: string; name: string; data: unknown; updatedAt?: number }[],
	) => ipcRenderer.invoke("cloud:push-presets", presets),
	cloudPullPresets: () => ipcRenderer.invoke("cloud:pull-presets"),
	cloudPushProjectMeta: (row: Record<string, unknown>) =>
		ipcRenderer.invoke("cloud:push-project-meta", row),
	onCloudStatusChanged: (callback: (status: CloudStatusBridge) => void) => {
		const listener = (_event: Electron.IpcRendererEvent, status: CloudStatusBridge) =>
			callback(status);
