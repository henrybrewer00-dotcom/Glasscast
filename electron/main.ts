...
import { pruneRecordingsTrash } from "./ipc/register/project";
import { shouldUseSyntheticLinuxPortalSource } from "./ipc/register/sourceMapping";
import { ensureMediaServer } from "./mediaServer";
import { migrateLegacyUserData } from "./migrateLegacyData";
...
const IS_SMOKE_EXPORT = process.env.GLASSCAST_SMOKE_EXPORT === "1";
...
function getPlatformAppIconFilename(size: 32 | 128 | 512) {
	const baseName = process.platform === "darwin" ? "glasscastmac" : "glasscast";
...
function getDefaultTrayIcon() {
	if (!defaultTrayIcon) {
		if (process.platform === "darwin") {
			// macOS menu bar icons must be monochrome template images so the
			// system tints them for light/dark menu bars; rendering the full
			// color app icon there reads as a white-ringed blob.
			const template = getAppImage("app-icons/trayTemplate.png");
			template.setTemplateImage(true);
			defaultTrayIcon = template;
		} else {
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Glasscast ${payload.version} is available`;
		case "downloading":
			return `Downloading Glasscast ${payload.version}`;
		case "ready":
			return `Glasscast ${payload.version} is ready`;
		case "error":
			return `Glasscast ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Glasscast.";
		case "downloading":
			return "Glasscast is downloading the update and will restart when it is ready.";
		case "ready":
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Glasscast";
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("dev.glasscast.app");
	}

	// One-time migration of legacy Recordly user data into the new Glasscast
	// userData directory. Runs before anything reads settings files.
	await migrateLegacyUserData();

	app.setAboutPanelOptions({
		applicationName: "Glasscast",
	...
	// Prune recordings discarded into `.trash` more than 7 days ago (issue #633).
	// Best-effort and self-contained — never blocks startup on failure.
	void pruneRecordingsTrash();
	...
	if (IS_SMOKE_EXPORT || process.env.GLASSCAST_DEV_OPEN_RECORDING_INPUT) {
		await logSmokeExportGpuDiagnostics();
		if (IS_SMOKE_EXPORT) {
			const smokeSource =
				process.env.GLASSCAST_SMOKE_EXPORT_PROJECT ??
				process.env.GLASSCAST_SMOKE_EXPORT_INPUT ??
				"<missing input>";
			console.log(`[smoke-export] Starting editor smoke export for ${smokeSource}`);
		} else {
			console.log(
				`[dev-open-recording] Starting editor for ${process.env.GLASSCAST_DEV_OPEN_RECORDING_INPUT}`,
			);
		}
		createEditorWindowWrapper();
		return;
