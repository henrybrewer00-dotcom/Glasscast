...
function getMarketplaceUrl(): string {
	...
	if (process.env.GLASSCAST_MARKETPLACE_URL) return process.env.GLASSCAST_MARKETPLACE_URL;
	return MARKETPLACE_API_BASE;
}

function getAdminKey(): string | undefined {
	return process.env.GLASSCAST_ADMIN_KEY;
}
...
async function marketplaceFetch<T>(
	...
	try {
		...
		if (options.admin) {
			...
			if (!key) throw new Error("Admin key not configured (set GLASSCAST_ADMIN_KEY env var)");
