	...
	ipcMain.handle(
		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			try {
				...
				if (
					!ALLOW_GLASSCAST_WINDOW_CAPTURE &&
					source?.id?.startsWith("window:") &&
					appName &&
					(appName === ownAppName || appName === "glasscast")
				) {
					return {
						success: false,
						message:
							"Cannot record Glasscast windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Glasscast needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Glasscast is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Glasscast needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Glasscast is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
