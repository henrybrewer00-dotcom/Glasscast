...
import {
	...
	TRASH_DIR_NAME,
} from "../../../src/lib/exporter/discardSafety";
...
export function registerProjectHandlers() {
  ...
  ipcMain.handle('save-project-file', async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string, thumbnailDataUrl?: string | null) => {
    try {
      ...
      const result = await dialog.showSaveDialog({
        title: 'Save Glasscast Project',
        defaultPath: path.join(projectsDir, defaultName),
        filters: [
          { name: 'Glasscast Project', extensions: [PROJECT_FILE_EXTENSION] },
  ...
  ipcMain.handle('load-project-file', async () => {
    try {
      const projectsDir = await getProjectsDir()
      const result = await dialog.showOpenDialog({
        title: 'Open Glasscast Project',
        defaultPath: projectsDir,
        filters: [
          { name: 'Glasscast Project', extensions: [PROJECT_FILE_EXTENSION, ...LEGACY_PROJECT_FILE_EXTENSIONS] },
          { name: 'JSON', extensions: ['json'] },
  ...
  ipcMain.handle('delete-recording-file', async (_, filePath: string) => {
    try {
      ...
      // Move discarded recordings into a recoverable `.trash` folder instead of
      // deleting them outright (issue #633 — a 2-hour recording was lost when
      // "Discard and Close" was pressed while it was still processing). The
      // `.trash` folder is pruned of entries older than 7 days on app start.
      ...
      // Also move the cursor telemetry sidecar if it exists so a restore keeps
      // cursor smoothing intact.
      const telemetryPath = getTelemetryPathForVideo(resolvedPath);
      const { destinationPath: telemetryDestinationPath } = computeTrashDestinationPath(
        telemetryPath,
        Date.now(),
      );
      await fs.rename(telemetryPath, telemetryDestinationPath).catch(() => undefined);
      ...
      return { success: true, trashedPath: destinationPath };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  });
...
/**
 * Prune discarded recordings from the recordings `.trash` folder once they are
 * older than {@link TRASH_RETENTION_MS} (7 days). Discarded recordings are moved
 * here instead of being permanently deleted (issue #633) so an accidental
 * "Discard and Close" stays recoverable for a week; this keeps the folder from
 * growing without bound. Best-effort — failures are logged, never thrown, so a
 * cleanup hiccup never blocks app startup.
 */
export async function pruneRecordingsTrash(nowMs: number = Date.now()): Promise<void> {
  try {
    const recordingsDir = await getRecordingsDir();
    const trashDir = path.join(recordingsDir, TRASH_DIR_NAME);
    const entries = await fs.readdir(trashDir).catch((error: NodeJS.ErrnoException) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (nowMs - stat.mtimeMs >= TRASH_RETENTION_MS) {
          await fs.rm(entryPath, { recursive: true, force: true });
        }
      } catch (entryError) {
        console.warn(`[recordings-trash] Failed to prune ${entryPath}:`, entryError);
      }
    }
  } catch (error) {
    console.warn("[recordings-trash] Failed to prune recordings trash:", error);
  }
}
