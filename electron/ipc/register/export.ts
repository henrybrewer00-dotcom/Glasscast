...
import { NativeExportHeartbeatEmitter } from "../export/nativeExportHeartbeat";
import {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				// Derive a renderer-side liveness signal from ffmpeg's own stderr/-progress
				// chatter so the export watchdog can tell "stuck on Preparing forever"
				// (issue #653) apart from "slowly making progress". Any line ffmpeg writes
				// while it is alive counts as liveness; the emitter throttles to ~1/s.
				const heartbeatEmitter = new NativeExportHeartbeatEmitter(sessionId);

				ffmpegProcess.stderr.on("data", (chunk: Buffer) => {
					const text = chunk.toString();
					session.stderrOutput += text;

					const heartbeat = heartbeatEmitter.ingest(text, Date.now());
					if (heartbeat && !event.sender.isDestroyed()) {
						event.sender.send("native-video-export-heartbeat", heartbeat);
					}
				});
