...
export const PROJECT_FILE_EXTENSION = "glasscast";
...
export const ALLOW_GLASSCAST_WINDOW_CAPTURE = Boolean(process.env["VITE_DEV_SERVER_URL"]);
export const RECORDING_SESSION_MANIFEST_SUFFIX = ".glasscast-session.json";
export const LEGACY_RECORDING_SESSION_MANIFEST_SUFFIXES = [".recordly-session.json"];
...
/**
 * Resolve the on-disk path for a downloaded whisper.cpp ggml model file.
 * Kept here (alongside WHISPER_MODEL_DIR) so the model registry and the
 * legacy `small` model path share a single directory convention.
 */
export function whisperModelPathForFile(fileName: string): string {
