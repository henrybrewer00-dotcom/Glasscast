...
const NVIDIA_CUDA_EXPORT_ENV = "GLASSCAST_EXPERIMENTAL_NVIDIA_CUDA_EXPORT";
const NVIDIA_CUDA_ALLOW_AUDIO_EXPORT_ENV = "GLASSCAST_NVIDIA_CUDA_ALLOW_AUDIO_EXPORT";
const NVIDIA_CUDA_FORCE_VIDEO_ONLY_ENV = "GLASSCAST_NVIDIA_CUDA_FORCE_VIDEO_ONLY";
const NVIDIA_CUDA_AUTO_STALL_TIMEOUT_ENV = "GLASSCAST_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS";
const DEFAULT_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS = 120_000;
const NATIVE_GPU_STALL_TIMEOUT_ENV = "GLASSCAST_NATIVE_GPU_STALL_TIMEOUT_MS";
...
function shouldPersistNativeExportDiagnostics() {
	const rawValue = process.env.GLASSCAST_NATIVE_EXPORT_DIAGNOSTICS?.trim().toLowerCase();
...
function shouldPersistNvidiaCudaExportDiagnostics() {
	return (
		...
		process.env.GLASSCAST_NVIDIA_CUDA_EXPORT_DIAGNOSTICS === "1"
	);
}
...
async function resolveExperimentalWindowsGpuExporterPath() {
	...
	const configuredPath = process.env.GLASSCAST_WINDOWS_GPU_EXPORT_EXE;
	if (configuredPath) {
...
export async function resolveExperimentalNvidiaCudaExportScriptPath() {
	...
	const candidates: string[] = [];
	const configuredPath = process.env.GLASSCAST_NVIDIA_CUDA_EXPORT_SCRIPT;
	if (configuredPath) {
		candidates.push(configuredPath);
	}
	const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
...
function resolveExperimentalNvidiaCudaNodeCommand() {
	const configuredNodePath = process.env.GLASSCAST_NVIDIA_CUDA_NODE_EXE;
...
export function buildExperimentalNvidiaCudaStaticLayoutArgs(
	...
	const args = [
		...
		"--stream-sync",
		"--prewarm-ms",
		process.env.GLASSCAST_NVIDIA_CUDA_PREWARM_MS || "500",
		"--content-x",
		String(Math.round(options.offsetX)),
		"--content-y",
		String(Math.round(options.offsetY)),
	...
	if (process.env.GLASSCAST_NVIDIA_CUDA_SAMPLE_GPU === "1") {
...
async function runExperimentalNvidiaCudaStaticLayoutExport(
	...
	const env = {
		...process.env,
		...nodeCommand.env,
		GLASSCAST_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY:
			process.env.GLASSCAST_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY ?? "1",
		GLASSCAST_FFMPEG_EXE: ffmpegPath,
		GLASSCAST_FFPROBE_EXE: process.env.GLASSCAST_FFPROBE_EXE ?? getFfprobeBinaryPath(),
