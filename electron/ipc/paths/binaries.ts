...
function getWindowsArchTag(): string {
	return process.arch === "arm64" ? "win32-arm64" : "win32-x64";
}

export function getPrebundledWindowsNativeHelperPath(binaryName: string): string {
	// Windows helpers always live under a win32-* arch tag, even when this code
	// runs on another host platform (e.g. unit tests on macOS).
	return resolveUnpackedAppPath("electron", "native", "bin", getWindowsArchTag(), binaryName);
}
