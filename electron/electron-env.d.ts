...
interface RendererNativeVideoExportHeartbeat {
	sessionId: string;
	sequence: number;
	atMs: number;
...
interface Window {
	electronAPI: {
		...
		onNativeVideoExportHeartbeat: (
			callback: (heartbeat: RendererNativeVideoExportHeartbeat) => void,
		) => () => void;
		...
		getWhisperModelStatus: (modelId?: string) => Promise<{
			success: boolean;
			modelId?: string;
			exists: boolean;
			path?: string | null;
		...
		getCaptionKeyStatuses: () => Promise<{
			success: boolean;
			statuses: CaptionKeyStatus[];
			error?: string;
		}>;
		getCaptionKeyStatus: (
			provider: string,
		) => Promise<{ success: boolean; status?: CaptionKeyStatus; error?: string }>;
		saveCaptionKey: (options: {
			provider: string;
			key: string;
		}) => Promise<{ success: boolean; status?: CaptionKeyStatus; error?: string }>;
		deleteCaptionKey: (
		...
		getAgentKeyStatuses: () => Promise<{
			success: boolean;
			statuses?: Array<{ provider: string; hasKey: boolean; last4: string | null }>;
			error?: string;
		}>;
		saveAgentKey: (options: { provider: string; key: string }) => Promise<{
			success: boolean;
			status?: { provider: string; hasKey: boolean; last4: string | null };
			error?: string;
		}>;
		deleteAgentKey: (provider: string) => Promise<{
			success: boolean;
			status?: { provider: string; hasKey: boolean; last4: string | null };
			error?: string;
		}>;
		...
		deleteRecordingFile: (
			filePath: string,
		) => Promise<{ success: boolean; trashedPath?: string; error?: string }>;
		getLocalMediaUrl: (
			filePath: string,
		...
		// ── Cloud sync & account ────────────────────────────────────────
		cloudGetStatus: () => Promise<{ success: boolean; status: RendererCloudStatus }>;
		cloudSignIn: (
			email: string,
			password: string,
		) => Promise<{ success: boolean; status?: RendererCloudStatus; error?: string }>;
		cloudSignUp: (
			email: string,
			password: string,
		) => Promise<{ success: boolean; status?: RendererCloudStatus; error?: string }>;
		cloudSignOut: () => Promise<{ success: boolean; status: RendererCloudStatus }>;
		cloudSyncNow: () => Promise<{
			success: boolean;
			status?: RendererCloudStatus;
			error?: string;
		}>;
		cloudNotifySettingsChanged: () => Promise<{ success: boolean }>;
		cloudPushPresets: (
			presets: { kind: string; name: string; data: unknown; updatedAt?: number }[],
		) => Promise<{ success: boolean; error?: string }>;
		cloudPullPresets: () => Promise<{
			success: boolean;
			presets?: { kind: string; name: string; data: unknown; updatedAt?: number }[];
			error?: string;
		}>;
		cloudPushProjectMeta: (
			row: Record<string, unknown>,
		) => Promise<{ success: boolean; error?: string }>;
		onCloudStatusChanged: (
			callback: (status: RendererCloudStatus) => void,
		) => () => void;
	};
}
