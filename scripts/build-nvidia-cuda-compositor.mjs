...
const videoCodecSdkRoot =
	process.env.GLASSCAST_NVIDIA_VIDEO_CODEC_SDK_ROOT?.trim() ||
...
if (!existsSync(path.join(videoCodecSdkRoot, "Samples", "NvCodec"))) {
	fallbackToBundledHelperOrExit(
		`NVIDIA Video Codec SDK samples not found at ${videoCodecSdkRoot}. Set GLASSCAST_NVIDIA_VIDEO_CODEC_SDK_ROOT to build from source.`,
	);
}
...
try {
	clearCmakeCache();
	execSync(
		`${cmake} .. -G "Visual Studio 17 2022" -A ${generatorArch} -DGLASSCAST_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
		{
			cwd: buildDir,
			stdio: "inherit",
			timeout: 120000,
	...
	try {
		clearCmakeCache();
		execSync(
			`${cmake} .. -G "Visual Studio 16 2019" -A ${generatorArch} -DGLASSCAST_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
			{
				cwd: buildDir,
				stdio: "inherit",
				timeout: 120000,
