...
import { readPersistedString } from "../persistedKeyMigration";
...
import type {
	...
	FrameInstance,
	GlasscastExtensionAPI,
	GlasscastExtensionModule,
	RenderHookContext,
	RenderHookFn,
	RenderHookPhase,
} from "./types";

const EXTENSION_SETTINGS_STORAGE_KEY = "glasscast.extension-settings.v1";
...
interface ActiveExtension {
	info: ExtensionInfo;
	module: GlasscastExtensionModule;
	disposables: (() => void)[];
}
...
export class ExtensionHost {
	...
	async activateExtension(info: ExtensionInfo, moduleUrl: string): Promise<void> {
		...
		let mod: GlasscastExtensionModule | null = null;
		try {
			...
			try {
				const loaded: GlasscastExtensionModule = await import(/* @vite-ignore */ moduleUrl);
				mod = loaded;
	...
	private readPersistedSettingsStore(): Record<string, Record<string, unknown>> {
		...
		try {
			const raw = readPersistedString(EXTENSION_SETTINGS_STORAGE_KEY);
			if (!raw) {
				return {};
			}
