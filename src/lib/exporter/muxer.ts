...
export class VideoMuxer {
	...
	async initialize(): Promise<void> {
		// Defensive: if this muxer was already initialized, discard the prior output
		// and its sources before building new ones. Otherwise a re-init would try to
		// re-add an already-connected source and mediabunny throws
		// "Source is already used for a track.".
		if (this.output) {
			try {
				await this.output.cancel();
			} catch {
				// already finalized/canceled — safe to discard
			}
			this.output = null;
