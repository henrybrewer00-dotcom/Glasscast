...
export class ModernVideoExporter {
	...
	private nativeExportHeartbeatUnsubscribe: (() => void) | null = null;
	...
	private exportWatchdog: ExportWatchdog | null = null;
	private watchdogTimer: ReturnType<typeof setInterval> | null = null;
	private watchdogProgressCounter = 0;
	private forceSoftwareFallback = false;
	private watchdogTrippedStage: ExportStage | null = null;
	...
	async export(): Promise<ExportResult> {
		...
		this.forceSoftwareFallback = false;
		this.watchdogTrippedStage = null;
		...
		while (true) {
			...
			try {
				...
				this.watchdogProgressCounter = 0;
				...
				// When the watchdog forced a software fallback on a previous attempt,
				// coerce the safest pipeline: software WebCodecs/libx264, no native
				// static-layout or native streaming encoder.
				const backendPreference: ExportBackendPreference = this.forceSoftwareFallback
					? "webcodecs"
				...
				this.startExportWatchdog("prepare");
				...
				// The stage-level watchdog tripped and tore down a stuck backend.
				// Retry once on the forced software fallback path before surfacing
				// any error (issue #653 — export stuck on Preparing forever).
				if (this.forceSoftwareFallback && this.watchdogTrippedStage && !watchdogRetried) {
					watchdogRetried = true;
					shouldRetryWithReadableFileSource = true;
					console.warn(
						`[VideoExporter] Watchdog tripped during ${this.watchdogTrippedStage}; retrying export on the software fallback pipeline.`,
						error,
					);
				} else if (
					...
					if (this.cancelled && !this.encoderError && !this.forceSoftwareFallback) {
						return {
							success: false,
							error: "Export cancelled",
							metrics: this.buildExportMetrics(),
					...
					if (this.forceSoftwareFallback && this.watchdogTrippedStage) {
						// Fallback path also failed — surface a clear, actionable error
						// rather than a silent hang.
						const stage = this.watchdogTrippedStage;
						const timeoutMs =
							this.exportWatchdog?.getStageTimeoutMs(stage) ??
							DEFAULT_STAGE_IDLE_TIMEOUT_MS;
						console.error("Export error (watchdog fallback exhausted):", error);
						return {
							success: false,
							error: describeExhaustedExport({ stage, timeoutMs }),
							metrics: this.buildExportMetrics(),
						};
					}
	...
	private async tryExportNativeStaticLayout(
		...
		const startedAt = this.getNowMs();
		const sessionId = `glasscast-static-layout-${Date.now()}-${Math.random()
			.toString(36)
			.slice(2, 8)}`;
	...
	private async tryStartNativeVideoExport(): Promise<boolean> {
		...
		// Subscribe to the native (ffmpeg) heartbeat so the stage watchdog gets a
		// liveness signal during the prepare phase, when no per-frame progress flows
		// over IPC yet (issue #653 — export stuck on "Preparing" forever).
		this.disposeNativeExportHeartbeatSubscription();
		this.nativeExportHeartbeatUnsubscribe =
			window.electronAPI.onNativeVideoExportHeartbeat?.((heartbeat) => {
				if (heartbeat.sessionId !== sessionId) {
					return;
				}
				this.handleNativeExportHeartbeat();
			}) ?? null;
	...
	private reportProgress(
		...
		this.observeExportWatchdog(phase);
	...
	/**
	 * Start the stage-level watchdog for the current export run. If any stage
	 * makes no progress for the configured idle timeout (default 90s) the active
	 * backend is considered stuck (issue #653) and — when a safe fallback has not
	 * yet been used — the run is aborted so the retry loop can re-run on the
	 * software path.
	 */
	private startExportWatchdog(initialStage: ExportStage): void {
		this.stopExportWatchdog();
		const stageIdleTimeoutMs =
			typeof this.config.stageIdleTimeoutMs === "number" &&
			Number.isFinite(this.config.stageIdleTimeoutMs) &&
			this.config.stageIdleTimeoutMs > 0
				? this.config.stageIdleTimeoutMs
				: DEFAULT_STAGE_IDLE_TIMEOUT_MS;
		this.exportWatchdog = new ExportWatchdog({
			stageIdleTimeoutMs,
			// Prepare can legitimately sit at 0% while the native helper spins up,
			// but #653 is precisely a prepare hang, so keep it bounded.
			stageTimeoutOverridesMs: { prepare: stageIdleTimeoutMs },
		});
		this.exportWatchdog.enterStage(initialStage, this.getNowMs());

		if (typeof setInterval !== "function") {
			return;
		}
		// Check at a fraction of the idle timeout so a trip is detected promptly.
		const pollMs = Math.max(1000, Math.floor(stageIdleTimeoutMs / 4));
		this.watchdogTimer = setInterval(() => {
			this.evaluateExportWatchdog();
		}, pollMs);
	}

	private evaluateExportWatchdog(): void {
		if (!this.exportWatchdog || this.cancelled) {
			return;
		}
		const status = this.exportWatchdog.check(this.getNowMs());
		if (status.kind !== "stuck") {
			return;
		}

		if (!status.fallbackAvailable) {
			// Already on the fallback path and still stuck — let the per-stage
			// finalization timeout / completion error surface naturally.
			console.error(
				`[VideoExporter] Watchdog: ${status.stage} stage stuck for ${Math.round(
					status.idleMs,
				)}ms on the fallback path; no safe retry remaining.`,
			);
			this.exportWatchdog.markAborted(true);
			return;
		}

		console.warn(
			`[VideoExporter] Watchdog: ${status.stage} stage made no progress for ${Math.round(
				status.idleMs,
			)}ms (timeout ${status.timeoutMs}ms). Aborting backend and retrying on the software fallback path.`,
		);
		this.watchdogTrippedStage = status.stage;
		this.forceSoftwareFallback = true;
		this.exportWatchdog.markAborted(true);
		// Tear down the stuck backend; the resulting error routes through the
		// retry loop, which re-runs with the software fallback forced on.
		this.cancel();
	}

	private observeExportWatchdog(phase: string): void {
		if (!this.exportWatchdog) {
			return;
		}
		const stage = mapExportPhaseToStage(phase);
		const snapshot = this.exportWatchdog.snapshot();
		if (snapshot.stage !== stage) {
			this.exportWatchdog.enterStage(stage, this.getNowMs());
		}
		// Monotonic counter guarantees each report counts as forward progress.
		this.watchdogProgressCounter += 1;
		this.exportWatchdog.observe(this.getNowMs(), this.watchdogProgressCounter);
	}

	/** Liveness signal forwarded from the native backend (ffmpeg heartbeat IPC). */
	handleNativeExportHeartbeat(): void {
		if (!this.exportWatchdog) {
			return;
		}
		this.exportWatchdog.observe(this.getNowMs());
	}

	private disposeNativeExportHeartbeatSubscription(): void {
		if (this.nativeExportHeartbeatUnsubscribe) {
			try {
				this.nativeExportHeartbeatUnsubscribe();
			} catch (error) {
				console.debug(
					"[VideoExporter] Ignoring error while unsubscribing native export heartbeat:",
					error,
				);
			}
			this.nativeExportHeartbeatUnsubscribe = null;
		}
	}

	private stopExportWatchdog(): void {
