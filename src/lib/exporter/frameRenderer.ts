...
export class FrameRenderer {
	...
	async renderFrame(
		...
		applyZoomTransform({
			...
			frameTimeMs: timeMs,
			zoomStyle: this.animationState.style,
			zoomIntensity: this.animationState.intensity,
		});
	...
	private updateAnimationState(timeMs: number): number {
		...
		state.style = region && targetProgress > 0 ? (region.style ?? "flat") : "flat";
		state.intensity = region?.intensity;
