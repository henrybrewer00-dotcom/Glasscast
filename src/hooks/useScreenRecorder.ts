...
import { buildDiscardRecoveryNote } from "@/lib/exporter/discardSafety";
...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!screenPermission.success || screenPermission.status !== "granted") {
			...
			alert(
				options.startup
					? "Glasscast needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Glasscast."
					: "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Glasscast before recording.",
			);
			return false;
		}
		...
		alert(
			options.startup
				? "Glasscast also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Glasscast."
				: "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Glasscast before recording.",
		);
		...
		try {
			...
			if (useLinuxPortal) {
				...
				if (cursorPresentation.nativeCaptureUnavailable) {
					console.warn(
						"Linux portal did not confirm cursor-hidden capture; disabling Glasscast cursor overlay for this recording.",
						{
							actualCursor,
							requestedCursor: browserCursorPolicy.streamCursor,
						},
		...
		if (nativeScreenRecording.current) {
			...
			void (async () => {
				try {
					const result = await window.electronAPI.stopNativeScreenRecording();
					if (result?.path) {
						const discardResult = await window.electronAPI.deleteRecordingFile(
							result.path,
						);
						// The screen recording is moved into a recoverable `.trash`
						// folder rather than deleted outright (issue #633), so let the
						// user know where to find it if this discard was a mistake.
						if (discardResult?.trashedPath) {
							toast.info(buildDiscardRecoveryNote(discardResult.trashedPath));
						}
					}
				} catch {
					// Best-effort cleanup
