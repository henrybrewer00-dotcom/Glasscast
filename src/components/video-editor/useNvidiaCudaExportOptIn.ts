...
import { legacyKeyFor } from "@/lib/persistedKeyMigration";

export const NVIDIA_CUDA_EXPORT_OPT_IN_SETTING_KEY =
	"glasscast.export.experimentalNvidiaCuda";

const LEGACY_NVIDIA_CUDA_EXPORT_OPT_IN_SETTING_KEY =
	legacyKeyFor(NVIDIA_CUDA_EXPORT_OPT_IN_SETTING_KEY) ??
...
export function loadInitialNvidiaCudaExportOptIn() {
	const current = loadAppSetting<boolean>(NVIDIA_CUDA_EXPORT_OPT_IN_SETTING_KEY);
	if (current !== null) {
		return current === true;
	}
	return loadAppSetting<boolean>(LEGACY_NVIDIA_CUDA_EXPORT_OPT_IN_SETTING_KEY) === true;
}
