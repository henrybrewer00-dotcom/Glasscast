...
const TimelineCanvasRows = memo(function TimelineCanvasRows({
	...
	// Running zebra index so row backgrounds alternate #0c0c0e / #0e0e10
	// across all row types in render order.
	let zebra = 0;

	return (
		<>
			<Row id={CLIP_ROW_ID} rowIndex={zebra++} isEmpty={clipItems.length === 0} hint={HINT_CLIP}>
			...
			{showSourceAudioTrack &&
				sourceAudioTracks.map((track) => (
					<Row key={track.id} id={`${SOURCE_AUDIO_ROW_ID}-${track.id}`} rowIndex={zebra++}>
						{clipItems
...
export default function TimelineCanvas({
	...
	return (
		<div
			...
			className="select-none bg-[#0c0c0e] relative cursor-pointer group flex flex-col"
		...
		>
			...
			{canShowGhostPlayhead && (
				...
				>
					<div
						className="absolute top-0 bottom-0 w-px bg-[#ff3b30]/45"
