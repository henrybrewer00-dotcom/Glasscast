...
const TimelineEditor = forwardRef<TimelineEditorHandle, TimelineEditorProps>(
	function TimelineEditor(
		...
		if (!videoDuration || videoDuration === 0) {
			// Lives inside the floating timeline dock: stay transparent and let the
			// dock wrapper own background, rounding, hairline and padding.
			return (
				<div className="flex-1 flex flex-col items-center justify-center gap-3">
					<div className="w-12 h-12 rounded-full border border-[#26262b] bg-[#131316] flex items-center justify-center">
						<Plus className="w-6 h-6 text-[#8a8a93]" />
					</div>
		...
		return (
			// Floating instrument dock: no full-bleed background, no attached-bottom
			// border, no edge assumptions. The dock wrapper (outside this file)
			// provides the rounded card, hairline and 8px internal padding; we just
			// fill it and clip our own scroll content to its rounded corners.
