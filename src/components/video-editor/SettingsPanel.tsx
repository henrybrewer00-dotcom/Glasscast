...
const CAPTION_PROVIDER_OPTIONS: Array<{ value: string; label: string; kind: "local" | "cloud" }> = [
	{ value: "glasscast", label: "Glasscast Cloud (Free)", kind: "cloud" },
	{ value: "local", label: "Local Whisper", kind: "local" },
	{ value: "openai", label: "OpenAI", kind: "cloud" },
	{ value: "groq", label: "Groq", kind: "cloud" },
	{ value: "deepgram", label: "Deepgram", kind: "cloud" },
];

/**
 * Per-model UI state for the local Whisper registry. Mirrors the rows returned
 * by `electronAPI.listWhisperModels()` plus live download status/progress that
 * the captions IPC streams back through `onCaptionModelDownloadProgress`.
 */
export interface WhisperModelUiState {
	id: string;
	label: string;
	sizeBytes: number;
...
/** Render an approximate on-disk model size (e.g. "488 MB", "1.5 GB"). */
...
const CAPTION_CLOUD_MODEL_OPTIONS: Record<string, Array<{ value: string; label: string }>> = {
	openai: [
...
const CAPTION_CLOUD_DEFAULT_MODEL: Record<string, string> = {
	openai: "whisper-1",
...
function SectionLabel({ children }: { children: React.ReactNode }) {
	return (
		<p className="text-engraved text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
			{children}
		</p>
	);
}
					...
					return (
						...
						>
							...
							<Switch
								...
								className="data-[state=checked]:bg-[var(--brand-accent)] scale-75"
							/>
						</div>
					);
				}
					...
					return (
						<button
							...
							className={cn(
								...
								isActive &&
									"border-[var(--brand-accent)]/70 bg-[var(--brand-accent)]/12 shadow-[inset_0_0_0_1px_rgba(var(--brand-accent-rgb),0.15)]",
							)}
						>
							<div className="flex items-start gap-3">
								<div
									className={cn(
										...
										isActive &&
											"border-[var(--brand-accent)]/30 bg-[var(--brand-accent)]/10 text-[#75A6FF]",
									)}
								>
		...
		<div className="flex flex-col gap-2">
			<div className="flex items-center justify-between">
				...
				<button
					...
					className="text-[10px] text-[var(--brand-accent)] transition-opacity hover:opacity-80"
					title={
						showAdvanced
							? tSettings(
					...
					return (
						<ToggleGroupItem
							...
							className={cn(
								...
								"data-[state=on]:border-[var(--brand-accent)]/70 data-[state=on]:bg-[var(--brand-accent)]/12 data-[state=on]:text-foreground",
							)}
						>
...
interface SettingsPanelProps {
	...
	/**
	 * Horizontal tab strip rendered at the top of the floating inspector card.
	 * When omitted, no strip renders (preserves the legacy left-docked callers).
	 */
	inspectorTabs?: InspectorTab[];
	/** Id of the active inspector tab. Drives which section content renders. */
	activeTab?: string;
	/** Invoked when the user picks a different inspector tab. */
	...
	selectedZoomStyle?: ZoomStyle | null;
	onZoomStyleChange?: (style: ZoomStyle) => void;
	selectedZoomIntensity?: number | null;
	...
	onKeyboardSoundPresetChange?: (preset: KeyboardSoundPreset) => void;
	mouseSoundPreset?: MouseSoundPreset;
	onMouseSoundPresetChange?: (preset: MouseSoundPreset) => void;
	agentExecuteTool?: AgentPanelProps["executeTool"];
	agentGetProjectSummary?: AgentPanelProps["getProjectSummary"];
	...
	whisperModels?: WhisperModelUiState[];
	...
	onDeleteWhisperModel?: (modelId: string) => void;
	selectedCaptionProvider?: string;
	onSelectedCaptionProviderChange?: (provider: string) => void;
	selectedCaptionModelId?: string;
	onSelectedCaptionModelIdChange?: (modelId: string) => void;
	captionKeyStatuses?: Record<string, { hasKey: boolean; last4: string | null }>;
	onSaveCaptionKey?: (provider: string, key: string) => void;
	onDeleteCaptionKey?: (provider: string) => void;
...
export function SettingsPanel({
	...
	keyboardSoundPreset = "off",
	onKeyboardSoundPresetChange,
	mouseSoundPreset = "off",
	onMouseSoundPresetChange,
	agentExecuteTool,
	...
	selectedCaptionProvider = "local",
	onSelectedCaptionProviderChange,
	selectedCaptionModelId = "small",
	onSelectedCaptionModelIdChange,
	captionKeyStatuses = {},
	onSaveCaptionKey,
	onDeleteCaptionKey,
	...
	// "glasscast" is a cloud provider but uses the free, login-based path, so it
	// never asks for an API key — only keyed cloud providers do.
	const isFreeCloudProvider = selectedCaptionProvider === "glasscast";
	const isKeyedCloudProvider =
		selectedCaptionProvider === "openai" ||
		selectedCaptionProvider === "groq" ||
		selectedCaptionProvider === "deepgram";
	const isCloudCaptionProvider = isKeyedCloudProvider || isFreeCloudProvider;
	const cloudModelOptions = isCloudCaptionProvider
		? (CAPTION_CLOUD_MODEL_OPTIONS[selectedCaptionProvider] ?? [])
		: [];
	const activeCloudKeyStatus = isKeyedCloudProvider
		? captionKeyStatuses[selectedCaptionProvider]
		: undefined;
	const selectedLocalModel = !isCloudCaptionProvider
		? whisperModels.find((model) => model.id === selectedCaptionModelId)
		: undefined;
	// Local captions can run when the chosen registry model is downloaded or the
	// user pointed at their own model file.
	const localModelReady = Boolean(
		whisperModelPath || selectedLocalModel?.status === "downloaded",
	);
	const [captionKeyDraft, setCaptionKeyDraft] = useState("");

	const handleCaptionProviderChange = (provider: string) => {
		onSelectedCaptionProviderChange?.(provider);
		setCaptionKeyDraft("");
		// Reset model selection to the provider's default so we never send a
		// model id that belongs to a different provider.
		if (CAPTION_CLOUD_DEFAULT_MODEL[provider]) {
			onSelectedCaptionModelIdChange?.(CAPTION_CLOUD_DEFAULT_MODEL[provider]);
		} else {
			onSelectedCaptionModelIdChange?.("small");
		}
	};
	...
	// The floating inspector drives section content through `activeTab` (reusing
	// the editor's existing section ids). Fall back to the legacy
	// `activeEffectSection` prop, then the internal default, so older callers that
	// never pass a tab strip keep working unchanged.
	const activeEffectSection =
		(activeTab as EditorEffectSection | undefined) ??
		activeEffectSectionProp ??
		internalActiveEffectSection;
	...
	const wallpaperTileClass = (isSelected: boolean) =>
		cn(
			"group relative aspect-square w-full overflow-hidden rounded-[6px] border bg-editor-bg transition-colors duration-150",
			isSelected
				? "border-2 border-[var(--brand-accent)] bg-foreground/[0.08]"
				: "border border-foreground/10 bg-foreground/[0.045] hover:border-foreground/20 hover:bg-foreground/[0.07]",
		);
								...
								>
									{isActive ? (
										<motion.span
											...
											className="absolute inset-0 rounded-lg bg-[var(--brand-accent)]"
											transition={{
												type: "spring",
												stiffness: 420,
												damping: 34,
							...
							{backgroundTab === "image" ? (
								<div className="mt-0 space-y-2">
									...
									<Button
										...
										variant="outline"
										className="w-full gap-2 bg-foreground/5 text-foreground border-foreground/10 hover:bg-[var(--brand-accent)] hover:text-white hover:border-[var(--brand-accent)] transition-all h-7 text-[10px]"
									>
	...
	if (isBackgroundPanel) {
		return (
			<div className="flex-[2] w-[332px] min-w-[280px] max-w-[332px] bg-editor-panel rounded-lg border border-border flex flex-col h-full overflow-hidden">
				<div
					className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 pb-24"
						...
						<div className="flex items-center justify-between">
							...
							{frame && (
								<button
									...
									className="text-[9px] text-[var(--brand-accent)] hover:opacity-80"
								>
									Remove
								</button>
							)}
								...
								return (
									<button
										...
										className={cn(
											...
											isSelected
												? "border-[var(--brand-accent)]/50 bg-[var(--brand-accent)]/10 ring-1 ring-[var(--brand-accent)]/30"
		...
		<section className="flex flex-col gap-2">
			...
			<div className="rounded-lg bg-foreground/[0.03] px-2.5 py-2 space-y-3">
				<div className="flex items-center justify-between gap-3">
					...
					<Select
						value={selectedCaptionProvider}
						onValueChange={handleCaptionProviderChange}
					>
						...
						<SelectContent className="border-foreground/10 bg-editor-surface-alt text-foreground">
							{CAPTION_PROVIDER_OPTIONS.map((option) => (
				...
				{isCloudCaptionProvider ? (
					<>
						...
						{isFreeCloudProvider ? (
							<div className="flex flex-col gap-1.5 rounded-xl border border-[var(--brand-accent)]/20 bg-[var(--brand-accent)]/5 px-3 py-2.5">
								<span className="text-sm font-medium text-foreground">
									{tSettings(
										"captions.freeCloudTitle",
										"Free AI captions — no API key",
									)}
								</span>
								<span className="text-xs text-muted-foreground">
									{tSettings(
										"captions.freeCloudHint",
										"Sign in to your free Glasscast account to generate captions in the cloud. Subject to fair-use limits.",
									)}
								</span>
							</div>
						) : (
					...
					<div className="flex flex-col gap-2">
							...
							<div className="flex items-center justify-between gap-2 rounded-xl border border-foreground/10 bg-foreground/5 px-3 py-2">
								<div className="flex min-w-0 flex-col">
									...
									<span className="text-[11px] text-muted-foreground">
										{formatModelSize(selectedLocalModel.sizeBytes)}
										{selectedLocalModel.status === "downloaded"
											? ` · ${tSettings("captions.modelDownloaded", "Downloaded")}`
											: ""}
									</span>
								</div>
								{selectedLocalModel.status === "downloading" ? (
									<span className="shrink-0 text-xs font-medium text-muted-foreground">
										...
										{Math.round(selectedLocalModel.progress)}%
									</span>
								) : selectedLocalModel.status === "downloaded" ? (
									<Button
										type="button"
						...
						{selectedLocalModel?.status === "downloading" ? (
							<div className="h-2 overflow-hidden rounded-full bg-foreground/5">
								<div
									...
									style={{ width: `${selectedLocalModel.progress}%` }}
								/>
							</div>
						) : null}
						<Button
						...
						>
							{whisperModelPath
								? tSettings("captions.customModelSelected", "Custom Model Selected")
				...
				<div className="flex flex-col gap-2">
					<Button
						...
						disabled={
							...
							(isFreeCloudProvider
								? false
								: isKeyedCloudProvider
									? !activeCloudKeyStatus?.hasKey
									: !localModelReady)
						}
						className="h-10 w-full rounded-xl bg-[var(--brand-accent)] px-4 text-sm font-medium text-white hover:bg-[var(--brand-accent)]/90 disabled:opacity-60"
					>
						{isGeneratingCaptions
	...
	const effectSectionContent = (() => {
		const settingsSectionContent = (
			<div className="space-y-4">
				...
				{showDevMotionControls ? (
					<section className="flex flex-col gap-2 rounded-xl border border-[var(--brand-accent)]/15 bg-[var(--brand-accent)]/5 p-3">
						<div className="flex items-center justify-between gap-3">
							<div>
								<SectionLabel>
						...
						<div className="rounded-lg border border-foreground/10 bg-background/60 px-3 py-3">
							<div className="flex items-start justify-between gap-3">
								...
								<Button
									...
									className="h-8 shrink-0 border-[var(--brand-accent)]/20 bg-[var(--brand-accent)]/10 text-[var(--brand-accent)] hover:bg-[var(--brand-accent)]/15"
								>
		...
		const soundSectionContent = (
			<div className="flex flex-col gap-2">
				<div className="flex items-center justify-between gap-3">
					<SectionLabel>{tSettings("sound.title", "Sound Effects")}</SectionLabel>
				</div>
				<div className="flex items-center justify-between rounded-lg bg-foreground/[0.03] px-2.5 py-1.5">
					<span className="text-[10px] text-muted-foreground">
						{tSettings("sound.enable", "Editor sound effects")}
					</span>
					<Switch
						checked={soundEffectsEnabled}
						onCheckedChange={(v) => onSoundEffectsEnabledChange?.(v)}
						className="data-[state=checked]:bg-[var(--brand-accent)] scale-75"
					/>
				</div>
				{soundEffectsEnabled && (
					<SliderControl
						label={tSettings("sound.volume", "Volume")}
						value={soundEffectsVolume}
						defaultValue={0.6}
						min={0}
						max={1}
						step={0.05}
						onChange={(v) => onSoundEffectsVolumeChange?.(v)}
						formatValue={(v) => `${Math.round(v * 100)}%`}
						parseInput={(text) => parseFloat(text.replace(/%$/, "")) / 100}
					/>
				)}
				<div className="flex items-center justify-between gap-3 rounded-lg bg-foreground/[0.03] px-2.5 py-1.5">
					<span className="text-[10px] text-muted-foreground">
						{tSettings("sound.keyboard", "Keyboard typing sound")}
					</span>
					<select
						value={keyboardSoundPreset}
						onChange={(e) =>
							onKeyboardSoundPresetChange?.(e.target.value as KeyboardSoundPreset)
						}
					...
					>
						{KEYBOARD_SOUND_PRESETS.map((preset) => (
				...
				<div className="flex items-center justify-between gap-3 rounded-lg bg-foreground/[0.03] px-2.5 py-1.5">
					<span className="text-[10px] text-muted-foreground">
						{tSettings("sound.mouse", "Mouse click sound")}
					</span>
					<select
			...
			<section className="flex flex-col gap-2">
				{selectedZoomId && (
					<>
						...
						{/* Cinematic camera-move style (Glasscast 3D zoom) */}
						<div className="mt-1">
							<span className="mb-1 block text-[10px] uppercase tracking-wider text-muted-foreground/70">
								{tSettings("zoom.styleLabel", "Camera style")}
							</span>
							<div className="grid grid-cols-3 gap-1.5">
								{ZOOM_STYLE_OPTIONS.map((option) => {
									const activeStyle = selectedZoomStyle ?? "flat";
									const isActive = activeStyle === option.value;
									return (
										<button
											key={option.value}
											type="button"
											title={option.description}
											onClick={() => onZoomStyleChange?.(option.value)}
											className={cn(
												"flex flex-col items-center gap-0.5 rounded-lg border px-1 py-2 text-center transition-all duration-200 ease-out cursor-pointer",
												isActive
													? "border-[var(--brand-accent)] bg-[var(--brand-accent)]/10 text-[var(--brand-accent)]"
													: "border-foreground/5 bg-foreground/5 text-muted-foreground hover:bg-foreground/10 hover:text-foreground",
											)}
										>
											<span className="text-xs font-semibold">
												{option.label}
							...
							<p className="mt-1.5 text-[10px] text-muted-foreground/70">
								{ZOOM_STYLE_OPTIONS.find(
									(o) => o.value === (selectedZoomStyle ?? "flat"),
								)?.description ?? ""}
							</p>
						</div>
						{(selectedZoomStyle ?? "flat") !== "flat" && (
							<div className="mt-1">
								<SliderControl
									label={tSettings("zoom.intensityLabel", "3D intensity")}
									value={selectedZoomIntensity ?? DEFAULT_ZOOM_3D_INTENSITY}
									defaultValue={DEFAULT_ZOOM_3D_INTENSITY}
									min={0}
									max={1}
									step={0.05}
		...
		switch (activeEffectSection) {
			case "ai":
				return agentExecuteTool && agentGetProjectSummary ? (
					<AgentPanel
						executeTool={agentExecuteTool}
						getProjectSummary={agentGetProjectSummary}
						onOpenKeys={() => onTabChange?.("account")}
					/>
				) : null;
			case "account":
				return <AgentKeyPanel />;
			case "settings":
												...
												return (
													<button
														...
														className={cn(
															...
															isSelected
																? "border-foreground/80 ring-1 ring-[var(--brand-accent)]/50"
									...
									>
										...
										{isActive ? (
											<motion.span
												...
												className="absolute inset-x-1.5 bottom-0 h-0.5 rounded-full bg-[var(--brand-accent)]"
