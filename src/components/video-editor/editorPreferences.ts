...
import { legacyKeyFor, readPersistedString } from "../../lib/persistedKeyMigration";
import {
...
export const EDITOR_PREFERENCES_STORAGE_KEY = "glasscast.editor.preferences";
export const EDITOR_PRESETS_STORAGE_KEY = "glasscast.editor.presets";

const LEGACY_EDITOR_PREFERENCES_STORAGE_KEY =
	legacyKeyFor(EDITOR_PREFERENCES_STORAGE_KEY) ?? "recordly.editor.preferences";
const LEGACY_EDITOR_PRESETS_STORAGE_KEY =
	legacyKeyFor(EDITOR_PRESETS_STORAGE_KEY) ?? "recordly.editor.presets";
...
const CAPTION_PROVIDER_IDS = ["local", "openai", "groq"] as const;

function normalizeCaptionProvider(value: unknown, fallback: string): string {
	if (
		typeof value === "string" &&
		(CAPTION_PROVIDER_IDS as readonly string[]).includes(value)
	) {
		return value;
	}
	return fallback;
...
function normalizeCaptionModelId(value: unknown, fallback: string): string {
	if (typeof value === "string" && value.trim().length > 0) {
		return value.trim();
	}
	return fallback;
...
export function normalizeEditorPreferences(
	...
	return {
		...
		selectedCaptionProvider: normalizeCaptionProvider(
			raw.selectedCaptionProvider,
			fallback.selectedCaptionProvider,
		),
		selectedCaptionModelId: normalizeCaptionModelId(
			raw.selectedCaptionModelId,
			fallback.selectedCaptionModelId,
		),
	};
}
...
export function loadEditorPreferences(): EditorPreferences {
	const persisted =
		loadAppSetting<unknown>(EDITOR_PREFERENCES_STORAGE_KEY) ??
		loadAppSetting<unknown>(LEGACY_EDITOR_PREFERENCES_STORAGE_KEY);
	if (persisted !== null) {
		return normalizeEditorPreferences(persisted);
	}

	try {
		const stored = readPersistedString(EDITOR_PREFERENCES_STORAGE_KEY);
		if (!stored) {
...
export function loadEditorPresets(): EditorPreset[] {
	const persisted =
		loadAppSetting<unknown>(EDITOR_PRESETS_STORAGE_KEY) ??
		loadAppSetting<unknown>(LEGACY_EDITOR_PRESETS_STORAGE_KEY);
	if (persisted !== null) {
		return normalizeEditorPresets(persisted);
	}

	try {
		const stored = readPersistedString(EDITOR_PRESETS_STORAGE_KEY);
		if (!stored) {
			return [];
		}
