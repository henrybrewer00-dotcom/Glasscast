...
export const GLASSCAST_ISSUES_URL = "https://github.com/henrybrewer00-dotcom/Glasscast/issues";
const CONTACT_EMAIL = "henrybrewer00@gmail.com";
const FEEDBACK_MAILTO = `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent("Glasscast feedback")}`;
...
export function FeedbackDialog() {
	...
	const sendFeedback = () =>
		void openExternalLink(FEEDBACK_MAILTO, t("feedback.openFailed", "Failed to open link."));
			...
			<DialogContent className="max-w-md bg-editor-dialog border-foreground/10 [&>button]:text-muted-foreground [&>button:hover]:text-foreground">
				<DialogHeader>
					<DialogTitle className="text-xl font-semibold text-foreground flex items-center gap-2">
						<MessageSquareMore className="h-5 w-5 text-[var(--brand-accent)]" /> {t("feedback.title", "Send feedback")}
					</DialogTitle>
					<DialogDescription className="text-muted-foreground">
						{t("feedback.description", "Tell us what is broken or missing — your message opens in your email app.")}
					</DialogDescription>
				</DialogHeader>
				<div className="mt-4 space-y-4">
					<div className="flex items-center justify-between gap-3 rounded-xl border border-foreground/10 bg-foreground/[0.03] px-3 py-3">
						<div className="min-w-0">
							<p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground/70">
								{t("feedback.emailLabel", "Email")}
							</p>
							<p className="mt-1 truncate text-sm font-medium text-foreground">{CONTACT_EMAIL}</p>
						</div>
					</div>
					<Button
						type="button"
						onClick={sendFeedback}
						className="h-11 w-full justify-between rounded-xl bg-[var(--brand-accent)] px-4 text-white shadow-[0_8px_24px_-8px_rgba(var(--brand-accent-rgb),0.6)] transition-colors hover:bg-[var(--brand-accent)]/92"
					>
						<span className="flex items-center gap-2 text-sm font-semibold">
							<MessageSquareMore className="h-4 w-4" />
							{t("feedback.reportIssue", "Email feedback")}
						</span>
	...
	return (
		<Dialog>
			...
			<DialogContent className="max-w-lg bg-editor-dialog border-foreground/10 [&>button]:text-muted-foreground [&>button:hover]:text-foreground">
				<DialogHeader>
					<DialogTitle className="text-xl font-semibold text-foreground flex items-center gap-2">
						<Keyboard className="h-5 w-5 text-[var(--brand-accent)]" /> {t("keyboardShortcuts.title")}
					</DialogTitle>
					<DialogDescription className="text-muted-foreground">
					...
					<div className="rounded-xl border border-foreground/10 bg-foreground/[0.03] p-4 space-y-2 text-xs">
						{SHORTCUT_ACTIONS.map((action) => (
							<div key={action} className="flex items-center justify-between gap-3 rounded-lg border border-foreground/5 bg-foreground/5 px-3 py-2.5">
								...
								<kbd className="rounded border border-foreground/10 bg-foreground/10 px-2 py-1 font-mono text-[var(--brand-accent)]">
						...
						<div className="grid grid-cols-1 gap-2 pt-2 sm:grid-cols-3">
							<div className="rounded-lg border border-foreground/5 bg-foreground/5 px-3 py-2.5">
								...
								<kbd className="mt-2 inline-flex rounded border border-foreground/10 bg-foreground/10 px-2 py-1 font-mono text-[var(--brand-accent)]">
									{scrollLabels.pan}
								</kbd>
							</div>
							<div className="rounded-lg border border-foreground/5 bg-foreground/5 px-3 py-2.5">
