...
const PhAccount = (props: { className?: string; weight?: "fill" | "regular" }) => (
	<User weight={props.weight ?? "regular"} className={props.className} />
);
const PhWand = (props: { className?: string; weight?: "fill" | "regular" }) => (
...
export default function VideoEditor() {
	...
	const [soundEffectsEnabled, setSoundEffectsEnabled] = useState<boolean>(
		() => loadAppSetting<boolean>("soundEffectsEnabled") ?? false,
	);
	const [soundEffectsVolume, setSoundEffectsVolume] = useState<number>(
		() => loadAppSetting<number>("soundEffectsVolume") ?? 0.6,
	);
	const [keyboardSoundPreset, setKeyboardSoundPreset] = useState<KeyboardSoundPreset>(
		() => loadAppSetting<KeyboardSoundPreset>("soundKeyboardPreset") ?? "off",
	);
	const [mouseSoundPreset, setMouseSoundPreset] = useState<MouseSoundPreset>(
	...
	const [selectedCaptionProvider, setSelectedCaptionProvider] = useState<string>(
		initialEditorPreferences.selectedCaptionProvider,
	);
	const [selectedCaptionModelId, setSelectedCaptionModelId] = useState<string>(
		initialEditorPreferences.selectedCaptionModelId,
	);
	const [captionKeyStatuses, setCaptionKeyStatuses] = useState<
		Record<string, { hasKey: boolean; last4: string | null }>
	>({});
	const [whisperModels, setWhisperModels] = useState<WhisperModelUiState[]>([]);
	...
	// Below this viewport width the inspector collapses into a right-side overlay
	...
	useEffect(() => {
		...
		const apply = (matches: boolean) => {
			setIsInspectorOverlay(matches);
			if (!matches) setInspectorOverlayOpen(false);
		};
		apply(query.matches);
	...
	// Discard-while-processing protection (issue #633): cancelling an in-flight
	// export requires the user to type the confirm phrase before the partial
	...
	// Late-defined handlers the AI executor calls (kept current via an effect below).
	const agentZoomDeleteRef = useRef<(id: string) => void>(() => {});
	const agentExportRef = useRef<() => void>(() => {});
	...
	// Marks a freshly-recorded video eligible for the smart end-trim, and which
	// video we've already applied it to (so it runs at most once per recording).
	const freshRecordingEndTrimPathRef = useRef<string | null>(null);
	const endTrimAppliedPathRef = useRef<string | null>(null);
	...
	// Tracks which recording we've already warned about for missing cursor data,
	// so the "no cursor captured" toast fires at most once per recording.
	const cursorTrackingWarnedPathRef = useRef<string | null>(null);
	...
	const editorSectionButtons = useMemo(
		() => [
			{ id: "ai" as const, label: t("settings.sections.ai", "AI"), icon: PhWand },
			...
			{
				id: "account" as const,
				label: t("settings.sections.aiKeys", "AI Keys"),
				icon: PhAccount,
			},
		],
	...
	// The floating inspector card renders a horizontal tab strip in place of the
	// old left icon rail. Reuse the exact section ids/labels/icons so every
	// existing handler and section content keeps working unchanged.
			...
			editorSectionButtons.map((section) => ({
				...
				icon: (
					<section.icon
						...
						weight={activeEffectSection === section.id ? "fill" : "regular"}
					/>
				),
			})),
		[editorSectionButtons, activeEffectSection],
	);
	...
	const projectDisplayName = useMemo(() => {
		...
		const withoutExtension = fileName
			.replace(/\.(glasscast|recordly|openscreen)$/i, "")
			.replace(/\.[^.]+$/, "");
			...
			try {
				...
				if (!smokeExportConfig.enabled && devOpenRecordingConfig.inputPath) {
					...
					pendingFreshRecordingAutoZoomPathRef.current = autoApplyFreshRecordingAutoZooms
						? sourceVideoUrl
						: null;
					freshRecordingEndTrimPathRef.current = sourceVideoUrl;
					setWebcam((prev) => ({
						...prev,
	...
	const refreshWhisperModels = useCallback(async () => {
		...
		setWhisperModels((current) =>
			result.models.map((model) => {
				// Preserve any in-flight download progress that a plain status
				// refresh would otherwise clobber back to idle.
				const existing = current.find((entry) => entry.id === model.id);
				if (existing?.status === "downloading" && !model.exists) {
					return { ...existing, label: model.label, sizeBytes: model.sizeBytes };
				}
				return {
					id: model.id,
					label: model.label,
					sizeBytes: model.sizeBytes,
					status: model.exists ? ("downloaded" as const) : ("idle" as const),
					progress: model.exists ? 100 : 0,
					path: model.path,
				};
			}),
		);
	...
	const handleDownloadWhisperModel = useCallback(
		async (modelId: string) => {
			const current = whisperModels.find((model) => model.id === modelId);
	...
	const handleDeleteWhisperModel = useCallback(
		async (modelId: string) => {
			...
			if (!result.success) {
				...
				setWhisperModels((models) =>
					models.map((model) =>
						model.id === modelId ? { ...model, status: "idle", progress: 0 } : model,
					),
				);
				return;
			}

			// Clear a manually-picked path that pointed at the deleted registry file.
			if (deletedPath) {
				setWhisperModelPath((currentPath) =>
					currentPath === deletedPath ? null : currentPath,
				);
			}
			setWhisperModels((models) =>
				models.map((model) =>
					model.id === modelId
						? { ...model, status: "idle", progress: 0, path: null }
						: model,
				),
			);
	...
	const refreshCaptionKeyStatuses = useCallback(async () => {
		const result = await window.electronAPI.getCaptionKeyStatuses();
		if (!result.success) {
			return;
		}
		const next: Record<string, { hasKey: boolean; last4: string | null }> = {};
		for (const status of result.statuses) {
			next[status.provider] = { hasKey: status.hasKey, last4: status.last4 };
		}
		setCaptionKeyStatuses(next);
	}, []);
	...
	const handleDeleteCaptionKey = useCallback(async (provider: string) => {
		...
		setCaptionKeyStatuses((prev) => ({
			...prev,
			[provider]: { hasKey: false, last4: null },
		}));
	...
	const handleGenerateAutoCaptions = useCallback(async () => {
		...
		const isLocalProvider = selectedCaptionProvider === "local";
		const selectedLocalModel = whisperModels.find(
			(model) => model.id === selectedCaptionModelId,
		);
		const selectedLocalModelReady = selectedLocalModel?.status === "downloaded";
		if (isLocalProvider && !whisperModelPath && !selectedLocalModelReady) {
			toast.error("Download the selected Whisper model or pick a model file first");
			return;
		}
		...
		try {
			const result = await window.electronAPI.generateAutoCaptions({
				videoPath: sourcePath,
				provider: selectedCaptionProvider,
				modelId: selectedCaptionModelId,
			...
			try {
				...
				if (mounted) {
					...
					if (shouldRetryFreshRecordingTelemetry) {
						...
						// Fresh recording, retries exhausted, still no usable cursor data:
						// surface it instead of silently producing a "dead" recording.
						pendingFreshRecordingAutoZoomPathRef.current === videoPath &&
						samples.length < 2 &&
						cursorTrackingWarnedPathRef.current !== videoPath
					) {
						cursorTrackingWarnedPathRef.current = videoPath;
						toast.warning("No cursor movement was captured for this recording.", {
							description:
								"Auto-zoom and cursor effects need cursor tracking. Give Glasscast Accessibility and Screen Recording access in System Settings › Privacy & Security, then record again.",
							duration: 14000,
						});
					}
				}
	...
	// Smart end auto-trim (post only): on a fresh, untouched recording, if the
	// cursor telemetry shows the user heading to the Stop control at the end, trim
	// that dead tail by shortening the last clip. It's a normal clip edit, so the
	// user can drag the edge to adjust or undo it. Never affects live recording.
	useEffect(() => {
		if (!videoPath || !isPreviewReady || duration <= 0) return;
		if (freshRecordingEndTrimPathRef.current !== videoPath) return;
		if (endTrimAppliedPathRef.current === videoPath) return;
		if (normalizedCursorTelemetry.length < 4) return;

		const totalMs = Math.round(duration * 1000);
		// Only act on the default single full-track clip (an untrimmed fresh take).
		if (clipRegions.length !== 1) return;
		const clip = clipRegions[0];
		if (clip.startMs !== 0 || clip.speed !== 1 || Math.abs(clip.endMs - totalMs) > 50) return;

		endTrimAppliedPathRef.current = videoPath;
		const suggestion = detectEndTrim(normalizedCursorTelemetry, totalMs);
		if (!suggestion) return;

		setClipRegions((prev) =>
			prev.map((region, index) =>
				index === prev.length - 1 ? { ...region, endMs: suggestion.trimStartMs } : region,
			),
		);
		const removedMs = totalMs - suggestion.trimStartMs;
		toast(`Auto-trimmed ${(removedMs / 1000).toFixed(1)}s off the end`, {
			description:
				"You were heading to Stop — drag the clip's right edge to adjust, or press ⌘Z to undo.",
			duration: 8000,
		});
	}, [videoPath, isPreviewReady, duration, normalizedCursorTelemetry, clipRegions]);
	...
	const handleZoomStyleChange = useCallback(
		(style: ZoomStyle) => {
			if (!selectedZoomId) return;
			setZoomRegions((prev) =>
				prev.map((region) =>
					region.id === selectedZoomId ? { ...region, style } : region,
				),
			);
			soundEffects.play(style === "flat" ? "click" : "pop");
		},
		[selectedZoomId],
	);
	...
	const handleZoomIntensityChange = useCallback(
		(intensity: number) => {
			...
			setZoomRegions((prev) =>
				prev.map((region) =>
					region.id === selectedZoomId
						? { ...region, intensity: Math.min(1, Math.max(0, intensity)) }
						: region,
				),
			);
		},
	...
	const handleSoundEffectsEnabledChange = useCallback((enabled: boolean) => {
		...
		saveAppSetting("soundEffectsEnabled", enabled);
	...
	const handleSoundEffectsVolumeChange = useCallback((volume: number) => {
		...
		saveAppSetting("soundEffectsVolume", volume);
		soundEffects.setVolume(volume);
	}, []);

	const handleKeyboardSoundPresetChange = useCallback((preset: KeyboardSoundPreset) => {
		setKeyboardSoundPreset(preset);
		saveAppSetting("soundKeyboardPreset", preset);
		soundEffects.setKeyboardSound(preset);
		if (preset !== "off") soundEffects.playKeyboard();
	}, []);

	const handleMouseSoundPresetChange = useCallback((preset: MouseSoundPreset) => {
		setMouseSoundPreset(preset);
		saveAppSetting("soundMousePreset", preset);
		soundEffects.setMouseSound(preset);
		if (preset !== "off") soundEffects.playMouse();
	}, []);

	// ── AI agent: read the project + execute tool calls against the live editor ──
	const agentGetProjectSummary = useCallback(() => {
		const durationMs = Math.round(duration * 1000);
		const playheadMs = Math.round(currentTime * 1000);
		const zoomDesc = zoomRegions.length
			? zoomRegions
					.map(
						(z) =>
							`${z.id} ${z.startMs}-${z.endMs}ms depth${z.depth} ${z.style ?? "flat"}`,
					)
					.join("; ")
			: "none";
		return [
			...
			`Zoom regions (${zoomRegions.length}): ${zoomDesc}.`,
			`Selected zoom: ${selectedZoomId ?? "none"}.`,
	...
	const agentExecuteTool = useCallback(
		async (call: ToolCall): Promise<{ ok: boolean; message: string }> => {
			const args = (call.args ?? {}) as Record<string, unknown>;
			const num = (v: unknown) => (typeof v === "number" ? v : Number(v));
			const clamp01 = (v: number) => Math.min(1, Math.max(0, Number.isFinite(v) ? v : 0.5));
			const asDepth = (v: unknown): ZoomDepth | null => {
				const n = num(v);
				return [1, 2, 3, 4, 5, 6].includes(n) ? (n as ZoomDepth) : null;
			};
			const asStyle = (v: unknown): ZoomStyle | null =>
				v === "flat" || v === "tilt3d" || v === "dolly" ? v : null;

			switch (call.name) {
				case "get_project_state":
					return { ok: true, message: agentGetProjectSummary() };
				case "add_zoom": {
					const startMs = num(args.startMs);
					const endMs = num(args.endMs);
					if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs <= startMs) {
						return { ok: false, message: "Invalid start/end times." };
					}
					const id = `zoom-${nextZoomIdRef.current++}`;
					const depth = asDepth(args.depth) ?? DEFAULT_ZOOM_DEPTH;
					const focus = {
						cx: clamp01(num(args.focusX ?? 0.5)),
						cy: clamp01(num(args.focusY ?? 0.5)),
					};
					...
					return {
						ok: true,
						message: `Added zoom ${id} (${region.startMs}-${region.endMs}ms, depth ${depth}${style ? `, ${style}` : ""}).`,
					};
				}
				case "update_selected_zoom": {
					if (!selectedZoomId) {
						return { ok: false, message: "No zoom selected. Add or select one first." };
					}
					const depth = asDepth(args.depth);
					const style = asStyle(args.style);
					setZoomRegions((prev) =>
						prev.map((z) =>
							z.id === selectedZoomId
								? {
										...
										...(style ? { style } : {}),
										...(args.intensity != null
											? { intensity: clamp01(num(args.intensity)) }
											: {}),
										...(args.focusX != null || args.focusY != null
											? {
													focus: {
														cx: clamp01(num(args.focusX ?? z.focus.cx)),
														cy: clamp01(num(args.focusY ?? z.focus.cy)),
													},
												}
											: {}),
									}
					...
					return { ok: true, message: `Updated zoom ${selectedZoomId}.` };
				}
				case "select_zoom": {
					const id = String(args.id);
					if (!zoomRegions.some((z) => z.id === id)) {
						return { ok: false, message: `No zoom with id ${id}.` };
					}
					setSelectedZoomId(id);
					return { ok: true, message: `Selected ${id}.` };
				}
				case "auto_suggest_zooms":
					if (cursorTelemetry.length < 2) {
						return { ok: false, message: "No cursor data available to suggest zooms." };
					}
					setAutoSuggestZoomsTrigger((v) => v + 1);
					return { ok: true, message: "Auto-suggesting zooms from cursor activity." };
				case "delete_selected":
					if (selectedZoomId) {
						agentZoomDeleteRef.current(selectedZoomId);
						return { ok: true, message: "Deleted the selected zoom." };
					}
					return { ok: false, message: "Nothing is selected to delete." };
				case "generate_captions":
					void handleGenerateAutoCaptions();
					return { ok: true, message: "Started generating captions." };
				case "set_background":
					setWallpaper(String(args.value ?? ""));
					return { ok: true, message: `Background set to ${args.value}.` };
				case "seek_to": {
					const ms = num(args.timeMs);
					const v = videoPlaybackRef.current?.video;
					if (v && Number.isFinite(ms)) {
						v.currentTime = ms / 1000;
						return { ok: true, message: `Moved playhead to ${ms}ms.` };
					}
					return { ok: false, message: "Could not seek." };
				}
				case "set_playback":
					if (args.playing) {
	...
	// Sync the shared SFX engine with the persisted preferences on mount.
	useEffect(() => {
		soundEffects.setEnabled(soundEffectsEnabled);
		soundEffects.setVolume(soundEffectsVolume);
		soundEffects.setKeyboardSound(keyboardSoundPreset);
		soundEffects.setMouseSound(mouseSoundPreset);
		// Intentionally run once; subsequent updates go through the handlers above.
	}, []);

	// Global keyboard-typing and mouse-click ambience. Each fires only when its
	// preset is set (independent of the master editor-SFX toggle).
	useEffect(() => {
		const onKeyDown = (e: KeyboardEvent) => {
			if (soundEffects.getKeyboardSound() === "off") return;
			if (e.metaKey || e.ctrlKey || e.altKey) return;
			// Only "typing" keys: single characters, space, enter, backspace, tab.
			const isTypingKey =
				e.key.length === 1 ||
				e.key === "Backspace" ||
				e.key === "Enter" ||
				e.key === "Tab" ||
				e.key === " ";
			if (isTypingKey && !e.repeat) soundEffects.playKeyboard();
		};
		const onPointerDown = (e: PointerEvent) => {
			if (soundEffects.getMouseSound() === "off") return;
	...
	useEffect(() => {
		...
		// When smoke-export opens a .glasscast project, the cursor telemetry
	...
	const performCancelExport = useCallback(() => {
	...
	const handleCancelExport = useCallback(() => {
		// While a render/finalize is active, require the typed-confirm gate before
		// discarding the in-flight export (issue #633). The first acknowledgement is
		// implicit in pressing Cancel; the dialog collects the typed phrase.
		const gate = evaluateDiscardGate({
			processingActive: isExporting,
			firstConfirmationAcknowledged: true,
			typedConfirmation: "",
		});
		if (!gate.allowed && gate.requiredTypedConfirmation) {
			setDiscardConfirmText("");
			setDiscardConfirmOpen(true);
			return;
		}
		performCancelExport();
	}, [isExporting, performCancelExport]);

	const handleConfirmDiscardExport = useCallback(() => {
		const gate = evaluateDiscardGate({
			processingActive: true,
			firstConfirmationAcknowledged: true,
			typedConfirmation: discardConfirmText,
		});
		if (!gate.allowed) {
			return;
		}
		...
		setDiscardConfirmText("");
		performCancelExport();
	}, [discardConfirmText, performCancelExport]);

	// Keep the AI executor's late-handler refs current.
	useEffect(() => {
		agentZoomDeleteRef.current = handleZoomDelete;
		agentExportRef.current = handleStartExportFromDropdown;
	});
				...
				<DialogHeader>
					...
					<DialogDescription className="text-muted-foreground">
						{t(
							...
							"Your device does not support native capture. This could be for a variety of reasons we haven’t figured out yet. This doesn’t break Glasscast, but it does make cursor smoothing impossible.",
						)}
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
	...
	const discardConfirmDialog = (
		...
		>
			<DialogContent className="max-w-md bg-editor-dialog border-foreground/10 text-foreground">
				<DialogHeader>
					<DialogTitle>
						{t("editor.discardConfirm.title", "Discard the export in progress?")}
					</DialogTitle>
					<DialogDescription className="text-muted-foreground">
						{t(
				...
				<Input
					...
					onChange={(event) => setDiscardConfirmText(event.target.value)}
					onKeyDown={(event) => {
						if (event.key === "Enter") {
							event.preventDefault();
							handleConfirmDiscardExport();
						}
					}}
				...
				<DialogFooter>
					<Button
						...
						className="border-foreground/10 bg-foreground/5 text-muted-foreground hover:bg-foreground/10"
					>
						{t("editor.discardConfirm.keepExporting", "Keep exporting")}
					</Button>
					<Button
						type="button"
						onClick={handleConfirmDiscardExport}
						disabled={
							!evaluateDiscardGate({
								processingActive: true,
								firstConfirmationAcknowledged: true,
					...
					>
						{t("editor.discardConfirm.confirm", "Discard export")}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	...
	if (loading) {
		return (
			<div className="flex h-screen items-center justify-center bg-background">
				...
				<Toaster className="pointer-events-auto" position="top-center" />
			</div>
		);
	}
	if (error) {
		...
		>
			<DropdownMenuTrigger asChild>
				<Button
					...
					className="relative inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[var(--brand-accent)] px-7 text-white shadow-[0_8px_30px_-6px_rgba(var(--brand-accent-rgb),0.6)] transition-colors duration-200 ease-out hover:bg-[var(--brand-accent-hover)] active:bg-[var(--brand-accent-pressed)]"
				>
			...
			<DropdownMenuContent
				...
				side="top"
				sideOffset={10}
				className="w-[360px] max-w-[calc(100vw-24px)] border-none bg-transparent p-0 shadow-none"
			>
				{isExporting ? (
						...
						<div className="mt-4 flex gap-2">
							{hasPendingExportSave ? (
								<Button
									...
									className="h-8 flex-1 rounded-[5px] bg-[var(--brand-accent)] text-xs font-semibold text-white hover:bg-[var(--brand-accent)]/92"
								>
	...
	return (
		<div className="relative flex h-screen w-screen overflow-hidden bg-editor-bg text-foreground selection:bg-[rgba(var(--brand-accent-rgb),0.3)]">
			{/* Draggable window region (behind everything, lets traffic-light area + void drag the window) */}
			<div
				className="pointer-events-none absolute inset-x-0 top-0 z-0 h-14"
				style={{ WebkitAppRegion: "drag" } as React.CSSProperties}
			/>

			{/* ── TOP-LEFT floating chip cluster ───────────────────────── */}
			<div
				className={`fixed left-3 top-3 z-30 flex max-w-[calc(100vw-1.5rem)] items-center gap-2 md:max-w-[calc(100vw-260px)] ${headerLeftControlsPaddingClass}`}
				style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
			>
				<div className="flex flex-shrink-0 items-center gap-1.5 rounded-[10px] border border-border bg-editor-panel/85 px-1.5 py-1 backdrop-blur-md">
					<Button
						type="button"
						variant="ghost"
						onClick={handleUndo}
						disabled={!canUndo}
						className="inline-flex h-7 w-7 items-center justify-center rounded-[6px] p-0 text-foreground transition-colors hover:bg-foreground/10 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40"
					...
					<Button
						...
						size="sm"
						onClick={handleOpenProjectBrowser}
						className="inline-flex h-7 w-7 items-center justify-center rounded-[6px] p-0 text-foreground transition-colors hover:bg-foreground/10 hover:text-foreground"
				...
				{/* Project title chip (click-to-rename) */}
				<div className="min-w-0 flex-shrink rounded-[10px] border border-border bg-editor-panel/85 px-1 py-1 backdrop-blur-md">
					{isEditingProjectName ? (
						<form
							onSubmit={(event) => void handleProjectNameSubmit(event)}
							className="flex max-w-[min(40vw,360px)] items-center gap-1.5 px-2 py-1"
						>
							{hasUnsavedChanges ? (
								<span className="size-1.5 shrink-0 rounded-full bg-[var(--brand-accent)]" />
							) : null}
							<input
								...
								className="min-w-[8ch] max-w-[min(32vw,300px)] bg-transparent font-mono text-[12px] uppercase tracking-[0.1em] text-foreground/95 outline-none placeholder:text-muted-foreground/60 disabled:cursor-wait"
								style={{ width: `${Math.max(projectNameDraft.length, 8)}ch` }}
						...
						<button
							...
							className="inline-flex max-w-[min(40vw,360px)] items-center gap-1.5 rounded-[7px] px-2 py-1 transition-colors hover:bg-foreground/5"
						...
						>
							...
							<span className="truncate font-mono text-[12px] uppercase tracking-[0.1em] text-foreground/85">
								{projectDisplayName}
							</span>
							<span className="shrink-0 font-mono text-[11px] uppercase tracking-[0.08em] text-muted-foreground/60">
								.glasscast
			...
			{/* ── TOP-RIGHT floating chip cluster ──────────────────────── */}
			<div
				className="fixed right-3 top-3 z-30 flex items-center gap-2"
				style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
			>
				<div className="flex items-center gap-1 rounded-[10px] border border-border bg-editor-panel/85 px-1.5 py-1 backdrop-blur-md">
					<Popover open={presetPopoverOpen} onOpenChange={setPresetPopoverOpen}>
						<PopoverTrigger asChild>
							<button
								...
								className="inline-flex items-center gap-1.5 rounded-[7px] px-2 py-1 text-sm font-medium tracking-tight text-foreground outline-none transition-colors hover:bg-foreground/10"
							>
								...
								>
									...
									<div className="flex items-center gap-2">
										...
										<Button
											type="submit"
											size="sm"
											className="h-9 rounded-xl bg-[var(--brand-accent)] px-3 text-white shadow-[0_2px_8px_-2px_rgba(var(--brand-accent-rgb),0.5)] transition-all duration-200 hover:bg-[var(--brand-accent)]/90"
										>
												...
												return (
													<div
														...
														className={cn(
															...
															isActive
																? "border-[rgba(var(--brand-accent-rgb),0.3)] bg-[rgba(var(--brand-accent-rgb),0.1)] text-foreground"
					...
					<div className="mx-0.5 h-5 w-px bg-foreground/10" />
					{/* Account avatar — opens the Account inspector tab */}
					<button
						type="button"
						onClick={() => setActiveEffectSection("account")}
						title={t("settings.sections.account", "Account")}
						aria-label={t("settings.sections.account", "Account")}
						className={cn(
							"inline-flex h-7 w-7 items-center justify-center rounded-full outline-none transition-colors hover:bg-foreground/10",
					...
					>
						<User
							weight={activeEffectSection === "account" ? "fill" : "regular"}
							className="h-[18px] w-[18px]"
						/>
					</button>
				</div>
			...
			{/* ── RESPONSIVE WORKSPACE GRID ─────────────────────────────
			    columns: [stage | inspector], rows: [stage/inspector | timeline/inspector].
			    The inspector spans both rows in column 2. Below 980px it collapses to an
			    overlay sheet and the stage + timeline expand to the full width. ── */}
			<div
				className="absolute inset-0 z-10 grid gap-3 p-3 pt-14"
				style={{
					gridTemplateColumns: isInspectorOverlay
						? "minmax(0,1fr)"
						: "minmax(0,1fr) auto",
					gridTemplateRows: "minmax(0,1fr) auto",
				}}
			>
				{/* ── STAGE cell (r1c1): canvas + overlaid transport capsule ── */}
				<div
				...
				>
					...
					<div
						className="flex flex-shrink-0 items-center justify-center gap-2 py-1.5"
						style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
					>
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button
									variant="ghost"
									size="sm"
									className="h-7 gap-1 rounded-full border border-border bg-editor-panel/80 px-2.5 text-xs text-muted-foreground backdrop-blur-md transition-all hover:bg-foreground/10 hover:text-foreground"
								>
						...
						<Button
							...
							className="h-7 gap-1.5 rounded-full border border-border bg-editor-panel/80 px-2.5 text-xs text-muted-foreground backdrop-blur-md transition-all hover:bg-foreground/10 hover:text-foreground"
						>
					...
					{/* Canvas — centered in the void */}
					<div
						className="relative flex min-h-0 w-full flex-1 items-center justify-center px-4"
						style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
					>
						<div
							className="relative"
						...
						{/* Floating transport capsule — overlaps the canvas bottom-center */}
						<div className="pointer-events-none absolute inset-x-0 bottom-3 z-20 flex justify-center">
							<div className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-border bg-editor-panel/90 px-3 py-1.5 backdrop-blur-md">
								...
								<div className="relative flex h-6 w-20 select-none items-center overflow-hidden rounded-full border border-foreground/[0.06] bg-editor-bg/80">
									<div
									...
									/>
									<div
										className="pointer-events-none absolute bottom-[18%] top-[18%] z-10 w-[2px] rounded-full bg-foreground/95 shadow-[0_0_10px_rgba(var(--brand-accent-rgb),0.28)]"
				...
				{/* ── RIGHT INSPECTOR (c2, spans both rows) ────────────────── */}
				<div
					className={
						isInspectorOverlay
							? `fixed inset-y-0 right-0 z-40 flex w-[clamp(280px,80vw,340px)] flex-col overflow-hidden border-l border-border bg-editor-panel/95 shadow-2xl backdrop-blur-md transition-transform duration-200 ease-out ${inspectorOverlayOpen ? "translate-x-0" : "pointer-events-none translate-x-full"}`
							: "flex min-h-0 w-[clamp(280px,24vw,340px)] flex-col overflow-hidden rounded-2xl border border-border bg-editor-panel/92 shadow-xl backdrop-blur-md"
					}
					style={
						isInspectorOverlay
				...
				{/* Backdrop for the inspector overlay sheet */}
				{isInspectorOverlay && inspectorOverlayOpen ? (
					<button
						type="button"
						aria-label={t("common.actions.close", "Close")}
						onClick={() => setInspectorOverlayOpen(false)}
			...
			{/* Inspector toggle FAB — only when the inspector is collapsed to an
			    overlay sheet and currently closed. */}
			{isInspectorOverlay && !inspectorOverlayOpen ? (
				<Button
					type="button"
					onClick={() => setInspectorOverlayOpen(true)}
					className="fixed right-3 top-14 z-30 inline-flex h-9 items-center gap-1.5 rounded-full border border-border bg-editor-panel/90 px-3 text-xs font-medium text-foreground shadow-lg backdrop-blur-md transition-colors hover:bg-foreground/10"
					style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
					title={t("settings.sections.title", "Inspector")}
					aria-label={t("settings.sections.title", "Inspector")}
				>
			...
			{/* Export pill — fixed bottom-right only when the inspector (which owns
			    the docked export pill) is hidden, so it never covers the timeline. */}
			{isInspectorOverlay && !inspectorOverlayOpen ? (
				<div
					className="fixed bottom-[calc(clamp(140px,22vh,220px)+24px)] right-3 z-30"
					style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
				>
					{exportMenu}
				</div>
