...
/**
 * Visual style of a zoom region's camera move.
 * - "flat"   – classic uniform 2D scale + pan (Recordly-style, default).
 * - "tilt3d" – cinematic perspective tilt toward the focus point (parallax / card-tilt).
 * - "dolly"  – subtle cinematic roll + punch-in for a "vertigo"-style push.
 */
export type ZoomStyle = "flat" | "tilt3d" | "dolly";
...
export interface ZoomRegion {
	...
	focus: ZoomFocus;
	mode?: ZoomMode;
	/** Camera-move style. Defaults to DEFAULT_ZOOM_STYLE when omitted. */
	style?: ZoomStyle;
	/** 0–1 strength of the 3D/dolly effect. Defaults to DEFAULT_ZOOM_3D_INTENSITY. */
	intensity?: number;
}
...
export const DEFAULT_CURSOR_CLICK_EFFECT_COLOR = "#FF3B30";
...
/** Default camera-move style for new zoom regions. */
export const DEFAULT_ZOOM_STYLE: ZoomStyle = "flat";
/** Default strength (0–1) of the 3D/dolly cinematic effect. */
export const DEFAULT_ZOOM_3D_INTENSITY = 0.6;

export const ZOOM_STYLE_OPTIONS: ReadonlyArray<{
	value: ZoomStyle;
	label: string;
	description: string;
}> = [
	{ value: "flat", label: "Flat", description: "Classic 2D punch-in" },
	{ value: "tilt3d", label: "3D Tilt", description: "Cinematic perspective toward focus" },
	{ value: "dolly", label: "Dolly", description: "Vertigo-style roll + push" },
];
