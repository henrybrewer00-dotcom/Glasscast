...
export function AnnotationOverlay({
	...
	const renderArrow = () => {
		...
		const color = annotation.figureData?.color || "#ff3b30";
	...
	return (
		<Rnd
			...
			className={cn(
				...
				isSelected && "ring-2 ring-[var(--brand-accent)] ring-offset-2 ring-offset-transparent",
			)}
			style={{
				zIndex: isSelectedBoost ? zIndex + 1000 : zIndex, // Boost selected annotation to ensure it's on top
				pointerEvents: "auto",
				border: isSelected ? "2px solid rgba(var(--brand-accent-rgb), 0.8)" : "none",
				backgroundColor: isSelected ? "rgba(var(--brand-accent-rgb), 0.1)" : "transparent",
				boxShadow: isSelected ? "0 0 0 1px rgba(var(--brand-accent-rgb), 0.35)" : "none",
			}}
			...
			resizeHandleStyles={{
				topLeft: {
					...
					border: isSelected ? "2px solid var(--brand-accent)" : "none",
					borderRadius: "50%",
					left: "-6px",
					top: "-6px",
					cursor: "nwse-resize",
