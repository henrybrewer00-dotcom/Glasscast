...
export function useHudBarDrag({
	...
	const handleHudBarPointerDown = useCallback(
		(event: PointerEvent<HTMLDivElement>) => {
			...
			// Never start a drag (and never preventDefault / capture the pointer) when the
			// press begins on an interactive control — otherwise the drag region swallows
			// the click and buttons/popovers in the strip stop working.
			const target = event.target as HTMLElement | null;
			if (
				target?.closest(
					'button, a, input, select, textarea, [role="button"], [role="menuitem"], [data-hud-interactive]',
				)
			) {
				return;
			}
