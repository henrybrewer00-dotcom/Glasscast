...
describe("ThemeContext persistence", () => {
	...
	it("loads the persisted theme preference from Electron app settings", () => {
		stubElectronSettings({ "glasscast.theme": "dark" });

		expect(loadThemePreference()).toBe("dark");
	});

	it("loads a legacy theme preference from Electron app settings", () => {
	...
	it("saves the theme preference to Electron app settings", () => {
		...
		expect(settingsStore.get("glasscast.theme")).toBe("dark");
	});

	it("falls back to localStorage when Electron settings are unavailable", () => {
		vi.stubGlobal(
			"localStorage",
			createStorageMock({ "glasscast.theme": "light" }),
		);

		expect(loadThemePreference()).toBe("light");
	});

	it("migrates the legacy localStorage theme key", () => {
		vi.stubGlobal(
			"localStorage",
